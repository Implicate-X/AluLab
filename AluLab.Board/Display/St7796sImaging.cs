...
namespace AluLab.Board.Display
{
	...
	{
		/// <summary>
		/// Send a bitmap to the ST7796S display specifying the starting position and destination clipping rectangle.
		/// </summary>
		...
		{
			if( _screenBuffer == null || _previousBuffer == null )
			{
				throw new InvalidOperationException( "Screen buffers are not initialized." );
			}

			if( forceFull )
			{
				SetWindow( 0, 0, ScreenWidth - 1, ScreenHeight - 1 );
				SendSPI( MemoryMarshal.Cast<Rgb565, byte>( _screenBuffer.AsSpan() ) );
			}
			else
			{
				int topRow;
	...
	reverse:
				...
				SetWindow( 0, topRow, w - 1, bottomRow );
